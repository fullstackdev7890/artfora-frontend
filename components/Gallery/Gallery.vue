<template>
  <div class="gallery" ref="galleryComponentRef">
    <div v-for="(column, index) in galleryImages" :key="index" class="col-20 col-large-25 col-laptop-33 col-tab-50"
      :class="{
      ...
      <nuxt-link v-for="(image, index) in column" :key="index" @click.prevent="(event) => clickProduct(event, image.id)"
        class="gallery-item">
        <div class="gallery-item-image-container" :class="{
          ...
          <user-details v-if="galleryViewType === DETAILS_GALLERY_VIEW_TYPE" :username="image.user.username"
            :author="image.author" :author-tag="image.user.tagname" :author-avatar="image.user.avatar_image" />
          <img
            :class="(!isAuthorized && image?.visibility_level > 1) || (isAuthorized && image?.visibility_level > product_visibility_level) ? 'blur-image' : ''"
            :src="getImageUrl(image.media[0], ImageTemplate.Thumbnail)" :alt="getTags(image)">
        </div>
      </nuxt-link>
    </div>
...
const getTags = (product: Product) => product.is_ai_safe ? product.tags : product.tags ? product.tags : randomWords(10).join(', ')
